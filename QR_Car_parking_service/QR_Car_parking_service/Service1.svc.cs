...
namespace QR_Car_parking_service
{
    ...
    public class Service1 : IService1
    {
        ...
        public Booking_info BookingID(string cust_id)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    SqlCommand cmd = new SqlCommand("select o_otp from otp_master where booking_id = @b_id", conn);
                    cmd.Parameters.AddWithValue("@b_id", booking_id);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        return new Booking_info
                        {
                            ...
                            otp = Convert.ToInt32(odt.Rows[0]["o_otp"]),
                            message = "Success"
                        };
                    }
                    else
            ...
            {
                return new Booking_info { message = ex.ToString() };
            }
        ...
        public respForget forgetPassword(string mail)
        {
            try
            {
                ...
                if (de.Rows.Count > 0)
                {
                    ...
                    catch (Exception ex) { return new respForget { msg = ex.ToString() }; }
        ...
        public respOtp checkParkingOtp(string otp)
        {
            try
            {
                conn = new SqlConnection(cs);
                SqlCommand cmd = new SqlCommand("select booking_id from otp_master where o_otp = @otp", conn);
            ...
            {
                return new respOtp { msg = ex.ToString() };
            }
        }
