package com.example.cp.otp_parking_app.Connection;

import android.content.Context;
import android.net.ConnectivityManager;
import android.net.NetworkInfo;

import com.example.cp.otp_parking_app.Data.Area_data;
import com.example.cp.otp_parking_app.Data.Booking_details;
import com.example.cp.otp_parking_app.Data.Cust_data;
import com.example.cp.otp_parking_app.Data.CustomerDetails;
import com.example.cp.otp_parking_app.Data.ForgotPass;
import com.example.cp.otp_parking_app.Data.Loc_point;
import com.example.cp.otp_parking_app.Data.LogPL;
import com.example.cp.otp_parking_app.Data.RatingDetails;
import com.example.cp.otp_parking_app.Data.RegistrationDetails;
import com.example.cp.otp_parking_app.Data.Slot_info;
import com.example.cp.otp_parking_app.Data.Slots_data;
import com.example.cp.otp_parking_app.Data.TransLog;
...
public class ConnectionM {
    ...
    public static String url5 = "http://demoproject.in/otpparking_service/Service1.svc/";
    ...
    public boolean fill_arealist() {
        try {
            ...
            if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
                ...
                ArrayList<String> stringArray, stringArray2, stringArray3;
    ...
    public boolean FillPark(String a_name) {
        try {
            ...
            if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
                ...
                br.close();
                JSONArray jarrayobj = new JSONArray(result.toString());
                ArrayList<String> stringArray1, stringArray2, stringArray3;
...
//http://192.168.1.6/QR_Car_parking_service/Service1.svc/login
    ...
    public String forgot(ForgotPass fp) {
        try{
            final String TAG_msg = "msg";
            StringBuilder result = new StringBuilder();

            HttpClient httpclient = new DefaultHttpClient();
            String url = String.format(url5 + "forgot");
            HttpPost httpPost = new HttpPost(url);
            String json = "";
            JSONObject jsonObject = new JSONObject();
            jsonObject.accumulate("email",fp.getEmail());
            ...
            if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
                ...
                if (msg.trim().equalsIgnoreCase("Please check your email"))
                    return "1";
                else
                    return "2";
            } else {
    ...
    public String checkParkingOtp(String otp) {
        try
        {
            StringBuilder result = new StringBuilder();
            String url = String.format(url5 + "/checkParkingOtp/"+otp);
            HttpClient httpclient = new DefaultHttpClient();
            HttpResponse response = httpclient.execute(new HttpGet(url));
            StatusLine statusLine = response.getStatusLine();
            if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
                ...
                if (!msg.equals("Success")) {
                    ...
                    CustomerDetails.setBookingId(booking_id);
                    return "2";
                }
            }
            else {
